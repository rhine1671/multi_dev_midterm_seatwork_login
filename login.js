...
regForm.addEventListener('submit', function (e) {
    ...
    //Validate if password are at least 8 character
	else if(passwordReg.value.length < 8) {
		alert("Password must have at least 8 characters");
	}

    // New validation for password containing only integers
    else if (/^\d+$/.test(passwordReg.value)) {
        alert("Password cannot only consist of numbers. Please use a combination of letters, numbers, and symbols.");
    }

	// Validate if password is not a combination of uppercase and lowercase characters
